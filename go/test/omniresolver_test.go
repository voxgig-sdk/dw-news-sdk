...
import (
	...
	omni "github.com/voxgig-sdk/dw-news-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DwNewsError); is {
				out := map[string]any{
					"name":    "DwNewsError",
...
func omniCtx(arg any, client *sdk.DwNewsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DwNewsSDK); is {
			client = live
