...
[MCP](https://modelcontextprotocol.io) server exposing the DwNews SDK as
two agent tools — `dw-news_list` and `dw-news_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dw-news-mcp)
make build

# 2. Provide credentials via the environment
export DW_NEWS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user dw-news \
  -- /absolute/path/to/dw-news-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./dw-news-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// dw-news_list: first page of records
{ "entity": "new" }
{ "entity": "new", "query": { } }

// dw-news_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dw-news-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user dw-news \
     -- "$PWD"/dist/*/dw-news-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `dw-news_list` and `dw-news_load` tools now appear
   in new sessions. Ask the agent to *"list new using dw-news"*
   and it calls `dw-news_list` with `{"entity":"new"}`.
...
```sh
export DW_NEWS_APIKEY=sk_live_xxx            # API key
export DW_NEWS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `dw-news_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `dw-news_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DW_NEWS_APIKEY` | API key sent with every request. |
| `DW_NEWS_BASE` | Optional override of the API base URL. |
...
```sh
./dw-news-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dw-news_load","arguments":{"entity":"new","query":{"id":1}}}}'
```
