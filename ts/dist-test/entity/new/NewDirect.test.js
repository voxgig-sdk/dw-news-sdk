...
(0, node_test_1.describe)('NewDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-new', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-new', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        const result = await client.direct({
            path: 'ru/breaking-news',
            method: 'GET',
            params,
            query,
        });
...
function liveScenariosActive() { return false && process.env.DW_NEWS_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.DwNewsSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['DW_NEWS_TEST_NEW_ENTID'];
    ...
    const client = new __1.DwNewsSDK({
        base: 'http://localhost:8080',
