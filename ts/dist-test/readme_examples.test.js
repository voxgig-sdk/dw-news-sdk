...
function isRunnable(code) {
    return (/new\s+DwNewsSDK\b/.test(code) ||
        /\bDwNewsSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+DwNewsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/DwNewsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
