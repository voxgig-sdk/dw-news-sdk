...
boru-driven command-line client **and** interactive REPL for the DwNews
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dw-news-cli)
make build
...
./dw-news-cli load 1 new            # {id:1} shorthand
./dw-news-cli load '{id:1}' new       # explicit match map

# 5. Override the API base URL for a single call
DW_NEWS_BASE=https://api.example.com ./dw-news-cli load 1 new
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dw-news-cli
   ```
...
```sh
./dw-news-cli load 1 new          # scalar shorthand for {id:1}
./dw-news-cli load '{id:1}' new     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `dw-news>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/dw-news-cli`. |
...
(`./dw-news-cli <expr>`) and interactively (the REPL), and expressions compose the
