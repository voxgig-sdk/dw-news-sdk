// Command DwNews-cli is a CLI + REPL for the DwNews SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DW_NEWS_APIKEY (api key), DW_NEWS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DW_NEWS_APIKEY carries the API key and
	// DW_NEWS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DW_NEWS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DW_NEWS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDwNewsSDK(opts)
