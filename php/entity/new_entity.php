...
class NewEntity
{
    ...
    {
        if ($args) {
            $this->_data = DwNewsHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = DwNewsHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     * Load a single New.
     *
     * @param NewLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed NewLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return New|array The loaded New as an assoc-array at the
     *   SDK boundary; throws DwNewsError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = DwNewsHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
