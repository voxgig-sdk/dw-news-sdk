...
// DwNews SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class DwNewsMakeError
{
    public static function call(?DwNewsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new DwNewsContext([], null);
        }
        $op = $ctx->op ?? new DwNewsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new DwNewsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof DwNewsError) ? $err->msg : (string)$err;
        $msg = "DwNewsSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new DwNewsError('', $msg, $ctx);
