...
class FtCtrl extends DwNewsControl
{
...
class FtClient
{
    ...
    private DwNewsUtility $utility;

    public function __construct(DwNewsUtility $utility, array $options)
    {
    ...
    public function get_utility(): DwNewsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DwNewsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new DwNewsUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (DwNewsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DwNewsConfig::make_feature($name);
    ...
    private static function buildUrl(DwNewsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DwNewsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DwNewsResponse($fetched);
        }
    ...
    private function fail(DwNewsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DwNewsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
