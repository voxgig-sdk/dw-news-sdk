...
{
    private static function utility(): DwNewsUtility
    {
        return new DwNewsUtility();
    }
    ...
    private static function ctx(array $over = []): DwNewsContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new DwNewsContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new DwNewsOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DwNewsError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DwNewsOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DwNewsMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DwNewsOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DwNewsMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DwNewsOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DwNewsMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DwNewsSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DwNewsMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new DwNewsResponse([]),
            'result' => new DwNewsResult([]),
        ]);
        [, $err] = DwNewsMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DwNewsSpec(['step' => 's']),
            'response' => new DwNewsResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DwNewsResult([]),
        ]);
        [, $err] = DwNewsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DwNewsSpec(['step' => 's']),
            'response' => new DwNewsResponse(self::resp(200, ['v' => 1])),
            'result' => new DwNewsResult([]),
        ]);
        [, $err] = DwNewsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DwNewsSpec(['step' => 's']),
            'response' => new DwNewsResponse(self::resp(200, ['v' => 2])),
            'result' => new DwNewsResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DwNewsMakeResponse::call($ctx);
    ...
    {
        $preset = new DwNewsResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DwNewsSpec([]),
            'response' => new DwNewsResponse([]),
            'result' => new DwNewsResult([]),
        ...
        [$got, $err] = DwNewsMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DwNewsResult([])]);
        [, $err] = DwNewsMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DwNewsSpec([]), 'result' => null]);
        [, $err] = DwNewsMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DwNewsSpec(['step' => 's']),
            'result' => new DwNewsResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DwNewsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DwNewsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DwNewsSpec(['step' => 's']),
            'result' => new DwNewsResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DwNewsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DwNewsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DwNewsResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DwNewsSpec([]),
            'result' => new DwNewsResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DwNewsMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DwNewsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new DwNewsError('boom', 'boom');
        $utility->fetcher = function (DwNewsContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DwNewsSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DwNewsMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DwNewsContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DwNewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DwNewsResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DwNewsContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DwNewsSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DwNewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DwNewsResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DwNewsSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DwNewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = DwNewsMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DwNewsMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DwNewsResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DwNewsDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DwNewsResult(['ok' => false])]);
        $this->expectException(DwNewsError::class);
        DwNewsDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DwNewsResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DwNewsMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DwNewsBaseFeature();
        $b = new DwNewsBaseFeature();
        DwNewsFeatureAdd::call($ctx, $a);
        DwNewsFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DwNewsBaseFeature
    {
        $f = new DwNewsBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        DwNewsFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): DwNewsContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DwNewsSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DwNewsPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new DwNewsResponse(['headers' => null]),
            'result' => new DwNewsResult([]),
        ]);
        DwNewsResultHeaders::call($ctx);
