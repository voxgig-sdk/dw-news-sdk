...
//    only - a typed DwNewsContext there would make every ctx
...
final class DwNewsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new DwNewsContext($clean, null);
    ...
    public static function err_from_map($m): ?DwNewsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new DwNewsError(is_string($code) ? $code : '', $msg);
    }
