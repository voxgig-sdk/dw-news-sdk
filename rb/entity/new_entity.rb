...
class NewEntity
  ...
  def data_set(args)
    if args
      @_data = DwNewsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = DwNewsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # Load a single New.
  #
  # @param reqmatch [NewLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.New.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [New, Hash] the loaded New; raises DwNewsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DwNewsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
