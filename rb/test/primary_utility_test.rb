...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = DwNewsOmni.make_runner(TEST_JSON_FILE, DwNewsSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : DwNewsOmni.errify(err)
    end
