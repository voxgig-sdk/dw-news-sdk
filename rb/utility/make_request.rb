# DwNews SDK utility: make_request
...
module DwNewsUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = DwNewsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DwNewsResponse.new(fetched)
    else
